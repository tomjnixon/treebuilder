...
handle_call({get_js, Id}, _From, State) ->
    F = fun() -> case mnesia:read(treebuilder_sketch, Id) of
                    [] -> null;
                    [#treebuilder_sketch{state=error}] -> null;
                    [#treebuilder_sketch{state=compiles, js_code=JsCode}] -> JsCode
                 end
        end,
